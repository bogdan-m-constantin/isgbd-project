...
namespace MiniDBMS.Utils
{
    ...
    {
        public static JoinResult ApplyCondition(this JoinResult rows, SelectCondition condition, Table table, SqlExecutionContext context, IDocumentSession session) {
            ...
            if (condition.Operand == "<")
                return rows.ApplyRangeCondition(column, int.MinValue, true, Convert.ToDecimal(condition.Value), false, context, table, session);
            if (condition.Operand == "<=")
                return rows.ApplyRangeCondition(column, int.MinValue, true, Convert.ToDecimal(condition.Value), true, context, table, session);
            if (condition.Operand == ">=")
                return rows.ApplyRangeCondition(column, Convert.ToDecimal(condition.Value), true, int.MaxValue, true, context, table, session);
            if (condition.Operand == ">")
                return rows.ApplyRangeCondition(column, Convert.ToDecimal(condition.Value), false, int.MaxValue, true, context, table, session);
            if (condition.Operand == "=")
        ...
        public static JoinResult ApplyRangeCondition(this JoinResult rows, Attribute column, decimal left, bool leftInclusive, decimal right, bool rightInclusive, SqlExecutionContext context, Table table, IDocumentSession session)
        {
            ...
            {
                Rows = rows.Rows.Where(e => ids.Contains(e[table.Name].Id)).ToList()
            };

        

    }
        public static JoinResult ApplyRangeConditionWithoutIndex(this JoinResult rows, Attribute column, decimal left, bool leftInclusive, decimal right, bool rightInclusive, SqlExecutionContext context, Table table, IDocumentSession session)
        {
            int leftCompare = leftInclusive ? 0 : 1;
            int rightCompare = rightInclusive ? 0 : -1;
        ...
        public static JoinResult ApplyEqualCondition(this JoinResult rows, Attribute column, string value, SqlExecutionContext context, Table table, IDocumentSession session)
        {
            ...
            return new JoinResult() { Rows = rows.Rows.Where(e => ids.Contains(e[table.Name].Id)).ToList() };

        }
        public static JoinResult ApplyEqualConditionWithoutIndex(this JoinResult rows, Attribute column, string value, SqlExecutionContext context, Table table, IDocumentSession session)
        {
        ...
        // !!!!!!!!!!!!!!!!!!!!!!
        public static JoinResult ApplyNotEqualCondition(this JoinResult rows, Attribute column, string value, SqlExecutionContext context, Table table, IDocumentSession session)
        {
            var index = table.GetIndexForColumn(column.Name);
            if (index == null)
        ...
        public static JoinResult ApplyNotEqualConditionWithoutIndex(this JoinResult rows, Attribute column, string value, SqlExecutionContext context, Table table, IDocumentSession session)
        {
        ...
        public static int CompareAs(DataType type, string a, string? b)
        {
            if (a == "NULL")
                return b == null ? 0 : -1;
